...
- 추가로 알아두면 좋은 것
  - RTC의 DataChannel을 이용하면 텍스트, 파일, 패킷과 같은 여러가지 데이터를 주고 받을 수있다.
    - `createDataChannel()`활용 하면 된다.
    - 해당 방법을 사용하면 Peer to Peer 방식이므로 중간 역활을 해주는 서버만 있으면 서버의 개입 없이도 파일 교환 같은것 또한 가능하다!! 서로 PC같 인터넷을 통한 연결을 하기 때문 !!
- 단점
  - 사람이 많아지면 성능상 이슈가 생길 수있다.
